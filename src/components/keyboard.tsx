...
// Memoized individual key to prevent re-renders
interface KeyboardKeyProps {
    letter: string;
    hint: GuessScore | undefined;
    isMobile: boolean;
    isLightMode: boolean;
    canInteract: boolean;
    pulseActive: boolean;
    feedbackEvaluation: GuessScore | undefined;
    feedbackDelay: number | undefined;
...
const KeyboardKey = memo(function KeyboardKey({
    ...
    const keyStyle: CSSProperties | undefined = feedbackDelay !== undefined
    ...
    return (
        <button
            ...
            className={cn(
                ...
                feedbackEvaluation && 'keyboard-feedback',
...
export const Keyboard = memo(function Keyboard({
    ...
    // Pre-compute feedback map for O(1) lookup instead of O(n) .find()
    const feedbackMap = React.useMemo(() => {
        if (!keyboardFeedback) return {};
        const map: Record<string, { evaluation: GuessScore; delay: number }> = {};
        for (const entry of keyboardFeedback.entries) {
            map[entry.letter] = { evaluation: entry.evaluation, delay: entry.delay };
        }
        return map;
    }, [keyboardFeedback]);
            ...
            {keyboardRows.map((row) => (
                ...
                >
                    {row.split('').map((letter) => {
                        const feedback = feedbackMap[letter];
                        return (
                            <KeyboardKey
                                key={letter}
                                letter={letter}
